...
description: Browse a Canvas Instructure Community KB (Knowledge Base) index, identify grading workflows that span Canvas and New Quizzes or Rollcall (Attendance) and are expressible through Canvas REST API calls, and produce Given-When-Then scenario files with a README. Use when the user asks to create an API-expressible grading scenario inventory from a Canvas KB feature area.
argument-hint: "[kb-index-url] <output-dir>"
disable-model-invocation: true
---

Browse the Canvas Instructure Community KB starting at `$0` (default: `https://community.instructure.com/en/kb/canvas-lms-instructor-guide`), identify grading workflows that involve Canvas and either New Quizzes or Rollcall (Attendance) and can be expressed through Canvas REST API calls, and produce a directory of Given-When-Then scenario files at `$1/`.

If `$0` is not provided or is the output directory path, treat the KB URL as `https://community.instructure.com/en/kb/canvas-lms-instructor-guide` and use the first argument as the output directory.
...
- **KB index page:** Category names (e.g., "Grades", "New Quizzes", "Attendance (Roll Call)") and article links are in the response. Collect every article title and URL from the relevant categories.
...
- **Notes blocks and warnings** — often document behaviors that differ between NQ/Rollcall and regular assignments (e.g., "grades for concluded enrollments are not sent")
...
- Applying rubrics to graded assignments backed by New Quizzes
- Grade posting and muting for quiz assignments

#### Gate 2 — Active boundary crossing

The external service (New Quizzes or Rollcall) must be **actively participating** in the `When` or `Then` — not merely providing a precondition in the `Given`.
...
| **Settings propagation** | A setting change in one system flows to the other (e.g., anonymous flag removed on grade post, exclude-from-grade flag set by Rollcall) |
...
- Canvas gradebook operations (post, hide, default grade, export, status change) applied to a quiz/attendance assignment
...
- Classic Quizzes (quiz engine v1) workflows — unless they also involve New Quizzes grade passback
...
2. **Triage decisions** — a table with columns: Article, In/Out of Scope, Grading-Related?, Integration (NQ/Rollcall/Neither), Active in When/Then? (Yes / No / ?), API-Expressible?, Reasoning. Mark anything borderline explicitly.
...
4. **Questions or ambiguities** — anything you're uncertain about (scope calls, domain groupings, API-expressibility you couldn't determine from the article alone)
...
**Integration:** Canvas LMS ↔ {New Quizzes | Rollcall (Attendance)}
...
1. **Opening paragraph** (no heading) — explain that these scenarios cover grading workflows at the integration boundary between Canvas LMS and New Quizzes / Rollcall (Attendance), and that all scenarios are expressible through the Canvas REST API
2. **Integration Boundary Map** — ASCII diagram showing Canvas LMS connected to New Quizzes and Rollcall:
...
3. **Scenario Files table** — columns: File, Domain, Source KB Articles, Integration (no scenario count)
4. **Scenario Schema** — copy the following section verbatim, including the Example block
5. **Notes** — include these Canvas-specific notes:
