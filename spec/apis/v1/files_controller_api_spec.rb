...
describe "Files API", type: :request do
  ...
  describe "GET /api/v1/files/:id (file-by-ID lookup)" do
    before :once do
      root_folder = Folder.root_folders(@course).first
      @attachment = Attachment.create!(filename: "secret.txt",
                                       display_name: "secret.txt",
                                       uploaded_data: StringIO.new("file"),
                                       folder: root_folder,
                                       context: @course)
      @api_path = "/api/v1/files/#{@attachment.id}"
      @api_opts = { controller: "files", action: "api_show", format: "json", id: @attachment.id.to_param }
    end
    ...
    it "rejects an authenticated low-privileged user with no access to the course even if files_visibility is public" do
      @course.update!(is_public: false, is_public_to_auth_users: false)
      @course.files_visibility = "public"
      @course.save!
    ...
    it "still honors an explicit verifier query parameter for an anonymous request on a private course" do
      @course.update!(is_public: false, is_public_to_auth_users: false)
      @course.files_visibility = "public"
      @course.save!
      @user = nil
      ...
      raw_api_call(:get,
                   "#{@api_path}?verifier=#{@attachment.uuid}",
                   @api_opts.merge(verifier: @attachment.uuid))
      expect(response).to have_http_status(:ok)
    end
  end

  describe "GET /api/v1/files/:id/public_url (file-by-ID lookup)" do
    before :once do
      root_folder = Folder.root_folders(@course).first
      @attachment = Attachment.create!(filename: "secret.txt",
                                       display_name: "secret.txt",
                                       ...
                                       folder: root_folder,
                                       context: @course)
      @api_path = "/api/v1/files/#{@attachment.id}/public_url"
      @api_opts = { controller: "files", action: "public_url", format: "json", id: @attachment.id.to_param }
    end
