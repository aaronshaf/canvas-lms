...
describe "context modules", :ignore_js_errors do
  ...
  context "module expand and collapse" do
    ...
    it "collapses all modules is retained on refresh" do # flaky-fix: QE-142, QE-161
      ...
      # expand all modules
      expand_all_modules_button.click
      wait_for_ajaximations # persist expand preference
      ...
      # collapse all modules
      collapse_all_modules_button.click
      wait_for_ajaximations # persist collapse preference before navigating away
  ...
  context "module header icons and progress" do
    ...
    it "shows a due date of the latest learning object due date when there is one" do
      Timecop.freeze(Time.zone.local(2025, 6, 16, 10, 5, 0)) do
        ...
        @assignment2.save!
        go_to_modules
        expect(module_header_due_date(@module1.id).text).to include(format_date_for_view(@assignment2.due_at, "%b %-d"))
      end
    end
