...
describe Mutations::DeleteConversations do
  ...
  describe "masquerade scope" do
    ...
    it "refuses to delete a cross-root-account conversation under masquerade" do
      mutation_command = <<~GQL
        mutation {
          deleteConversations(input: { ids: [#{@cross_ra.conversation_id}] }) {
            conversationIds
      ...
      errors = result.dig("data", "deleteConversations", "errors") || []
