...
describe SubmissionSearch do
  ...
  let(:amanda_principal) { Canvas::AdheresToPolicy::UserPrincipal.new(amanda) }
  let(:observer_principal) { Canvas::AdheresToPolicy::UserPrincipal.new(observer) }
  ...
  it "finds all submissions" do
    results = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "username" }]).search
  ...
  it "optionally includes deactivated students" do
    course.enrollments.find_by(user: jonah).deactivate
    results = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "username" }], include_deactivated: true).search
  ...
  it "optionally includes concluded students" do
    course.enrollments.find_by(user: jonah).conclude
    results = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "username" }], include_concluded: true).search
    expect(results.where(user: jonah).exists?).to be true
  end
  ...
  context "completed courses" do
    ...
    let(:completed_teacher_principal) { Canvas::AdheresToPolicy::UserPrincipal.new(completed_teacher) }
  ...
  it "filters for the specified workflow state" do
    ...
    results = SubmissionSearch.new(assignment, teacher_principal, nil, states: ["submitted"]).search
  ...
  it "filters results to specified sections" do
    ...
    results = SubmissionSearch.new(assignment, teacher_principal, nil, section_ids: [section.id]).search
    expect(results).to eq [Submission.find_by(user: amanda)]
  end

  it "filters by the enrollment type" do
    fake_student = assignment.course.student_view_student
    results = SubmissionSearch.new(assignment, teacher_principal, nil, enrollment_types: ["StudentEnrollment"]).search
  ...
  it "filters by scored less than" do
    ...
    results = SubmissionSearch.new(assignment, teacher_principal, nil, scored_less_than: 42).search
  ...
  it "filters by scored greater than" do
    assignment.grade_student(amanda, score: 42, grader: teacher)
    assignment.grade_student(mandy, score: 10, grader: teacher)
    results = SubmissionSearch.new(assignment, teacher_principal, nil, scored_more_than: 10).search
    expect(results).to eq [Submission.find_by(user: amanda)]
  end
  ...
  it "filters by late" do
    ...
    results = SubmissionSearch.new(assignment, teacher_principal, nil, late: true).search
    expect(results).to eq [submission]
  end

  it "filters by needs_grading" do
    submission = assignment.submit_homework(amanda, body: "asdf")
    results = SubmissionSearch.new(assignment, teacher_principal, nil, grading_status: "needs_grading").search
    expect(results).to eq [submission]
  end
  ...
  it "filters by excused" do
    ...
    submission.save!
    results = SubmissionSearch.new(assignment, teacher_principal, nil, grading_status: "excused").search
    expect(results).to eq [submission]
  end
  ...
  it "filters by needs_review" do
    ...
    submission.save!
    results = SubmissionSearch.new(assignment, teacher_principal, nil, grading_status: "needs_review").search
    expect(results).to eq [submission]
  end
  ...
  it "filters by graded" do
    ...
    submission.save!
    results = SubmissionSearch.new(assignment, teacher_principal, nil, grading_status: "graded").search
    expect(results).to eq [submission]
  end

  it "limits results to just associated student submissions if the user is an observer" do
    results = SubmissionSearch.new(assignment, observer_principal, nil, {}).search
    expect(results).to eq [Submission.find_by(user: amanda)]
  end

  it "limits results to just the user's submission if the user is a student" do
    results = SubmissionSearch.new(assignment, amanda_principal, nil, {}).search
    expect(results).to eq [Submission.find_by(user: amanda)]
  end

  it "returns nothing to randos" do
    rando = User.create!
    rando_principal = Canvas::AdheresToPolicy::UserPrincipal.new(rando)
    results = SubmissionSearch.new(assignment, rando_principal, nil, {}).search
    expect(results).to eq []
  end
  ...
  describe "final sort criteria (or, default sort criteria if no sorts are provided)" do
    it "orders students by user id by default for non-anonymized assignments" do
      results = SubmissionSearch.new(assignment, teacher_principal, nil, {}).search
  ...
  describe "username ordering" do
    ...
    it "returns students ordered by their sortable name (case insensitive), descending" do
      results = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "username", direction: "descending" }]).search
    ...
    it "returns students ordered by their 'anonymous name' when the assignment has NQ anonymous participants, ascending" do
    ...
    it "returns students ordered by their 'anonymous name' when the assignment has NQ anonymous participants, descending" do
      assignment.update!(anonymous_participants: true)
      results = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "username", direction: "descending" }]).search
      anon_names = results.map { |sub| assignment.anonymous_student_identities.dig(sub.user_id, :name) }
      expect(anon_names).to eql ["Student 5", "Student 4", "Student 3", "Student 2", "Student 1"]
  ...
  describe "username_first_last ordering" do
    it "returns students ordered by first names (case insensitive), ascending" do
      results = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "username_first_last" }]).search
    ...
    it "returns students ordered by first names (case insensitive), descending" do
      results = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "username_first_last", direction: "descending" }]).search
  ...
  describe "group name ordering" do
    ...
    it "returns students ordered by their group name (case insensitive, students without group always last), ascending" do
      results = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "group_name" }, { field: "username_first_last" }]).search
    ...
    it "returns students ordered by their group name (case insensitive, students without group always last), descending" do
      results = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "group_name", direction: "descending" }, { field: "username_first_last" }]).search
  ...
  it "supports sorting submissions by whether the user is the test student, ascending" do
    ...
    results = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "test_student" }, { field: "username" }]).search
  ...
  it "supports sorting submissions by whether the user is the test student, descending" do
    ...
    results = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "test_student", direction: "descending" }, { field: "username" }]).search
  ...
  it "returns a random-but-consistent order when given 'random' sort order" do
    first_result = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "random" }]).search.extract_associated(:user)
    second_result = SubmissionSearch.new(assignment, teacher_principal, nil, order_by: [{ field: "random" }]).search.extract_associated(:user)
  ...
  it "orders by submission score" do
    ...
    results = SubmissionSearch.new(assignment, teacher_principal, nil, scored_more_than: 0, order_by: [{ field: "score" }]).search
  ...
  it "orders by submission date" do
    Timecop.freeze do
      ...
      results = SubmissionSearch.new(assignment, teacher_principal, nil, states: "submitted", order_by: [{ field: "submitted_at" }]).search
  ...
  context "searchers with limited section visibility" do
    ...
    let(:teacher_section2_principal) { Canvas::AdheresToPolicy::UserPrincipal.new(teacher_section2) }
  ...
  context "group assignments" do
    ...
    describe "user_id" do
      it "returns the group rep's submission when provided with the group rep's ID" do
        results = SubmissionSearch.new(assignment, teacher_principal, nil, user_id: jonah.id).search
        aggregate_failures do
      ...
      it "returns the group rep's submission when provided with a different group member's ID" do
        results = SubmissionSearch.new(assignment, teacher_principal, nil, user_representative_id: amanda.id).search
        aggregate_failures do
          expect(results.count).to eq 1
      ...
      it "returns empty submissions if the user is searching for another group member user_representative_id and cannot view all grades" do
        results = SubmissionSearch.new(assignment, amanda_principal, nil, user_representative_id: jonah.id).search
        aggregate_failures do
    ...
    describe "representatives_only" do
      ...
      it "optionally returns only the group rep's submission" do
        results = SubmissionSearch.new(assignment, teacher_principal, nil, representatives_only: true).search
  ...
  describe "#filter_section_enrollment_states" do
    ...
    it "returns user_scope unchanged when user_scope is empty" do
      ...
      search = SubmissionSearch.new(assignment, teacher_principal, nil, apply_gradebook_enrollment_filters: true)
    ...
    it "returns user_scope unchanged when apply_gradebook_enrollment_filters is false" do
      ...
      search = SubmissionSearch.new(assignment, teacher_principal, nil, {})
