...
describe Assignment do
  ...
  describe "#enrollment_active_for_assignment?" do
    ...
    it "returns true when assignment is in a module with ADHOC override for the student" do
    ...
    it "returns true when assignment is in a module with Course override" do
      @assignment.update!(only_visible_to_overrides: true)

      context_module = @course.context_modules.create!(name: "test module")
      context_module.add_item(type: "assignment", id: @assignment.id)
      context_module.assignment_overrides.create!(set_type: "Course", set: @course)
  ...
  describe "context_module_tag_info" do
    ...
    it "returns past_due if an assignment is due in the past and no submission exists" do
      info = @assignment.context_module_tag_info(@student.principal, @course, has_submission: false)
    ...
    it "does not return past_due for assignments that were turned in on time" do
      ...
      info = @assignment.context_module_tag_info(@student.principal, @course, has_submission: true)
      expect(info[:past_due]).to be_falsey
    end
