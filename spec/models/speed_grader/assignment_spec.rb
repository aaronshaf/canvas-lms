...
describe SpeedGrader::Assignment do
  ...
  context "create and publish a course with 2 students" do
    ...
    context "add students to the group" do
      ...
      describe "only shows group comments" do
        ...
        before do
          json = SpeedGrader::Assignment.new(assignment, teacher.principal).json
  ...
  it "includes comments' created_at" do
    ...
    json = SpeedGrader::Assignment.new(@assignment, @user.principal).json
  ...
  it "returns provisional grade ids to provisional grader" do
    ...
    json = SpeedGrader::Assignment.new(assignment, @teacher.principal, grading_role: :provisional_grader).json
    expect(
  ...
  context "rubric association" do
    ...
    let(:json) { SpeedGrader::Assignment.new(@assignment, @user.principal).json }
  ...
  context "discussions assignment" do
    ...
    let(:json) { SpeedGrader::Assignment.new(@checkpointed_assignment, @teacher.principal).json }
  ...
  context "with submissions" do
    ...
    context "DocViewer" do
      ...
      let(:json) { SpeedGrader::Assignment.new(assignment, @teacher.principal).json }
      let(:sub) do
    ...
    describe "attachment JSON" do
      ...
      context "for an anonymized assignment" do
        ...
        it "includes the viewed_at field if the user is an admin" do
          ...
          json = SpeedGrader::Assignment.new(assignment, admin.principal).json
  ...
  describe "N+1 query prevention for attachment associations" do
    it "preloads last_attachment_upload_status and folder to prevent N+1 queries" do
      ...
      SpeedGrader::Assignment.new(assignment, @teacher.principal).json
  ...
  context "quizzes" do
    ...
    context "with quiz_submissions" do
      ...
      it "doesn't include quiz_submissions when there are too many attempts" do
        ...
        json = SpeedGrader::Assignment.new(@quiz.assignment, @teacher.principal).json
      ...
      it "returns quiz history for records before and after namespace change" do
        ...
        json = SpeedGrader::Assignment.new(@assignment.reload, @teacher.principal).json
        expect(json[:submissions].first["submission_history"].size).to eq 1
      end
  ...
  context "quizzes.next" do
    ...
    context "with quiz_submissions" do
      ...
      it "returns submission json correctly" do
        json = SpeedGrader::Assignment.new(@assignment, @student.principal).json
  ...
  describe "grader comment visibility" do
    ...
    context "when the user is the final grader" do
      let(:json) do
        SpeedGrader::Assignment.new(assignment, final_grader.principal, avatars: true, grading_role: :moderator).json
      end
    ...
    context "when the user is not the final grader and can view other grader comments" do
      let(:json) do
        SpeedGrader::Assignment.new(assignment, teacher.principal, avatars: true, grading_role: :provisional_grader).json
      end
  ...
  describe "moderated grading" do
    ...
    it "includes all provisional comments when grades have not been posted" do
      json = SpeedGrader::Assignment.new(assignment, second_ta.principal, grading_role: :provisional_grader).json
    ...
    it "creates a non-annotatable DocViewer session when the user cannot adjudicate" do
      ...
      json = SpeedGrader::Assignment.new(assignment, other_ta.principal, grading_role: :provisional_grader).json
    ...
    context "for provisional grader" do
      let(:json) { SpeedGrader::Assignment.new(assignment, ta.principal, grading_role: :provisional_grader).json }
    ...
    context "for final grader" do
      let(:json) { SpeedGrader::Assignment.new(assignment, teacher.principal, grading_role: :moderator).json }
  ...
  context "when an assignment is anonymous" do
    ...
    let(:speed_grader_json) { SpeedGrader::Assignment.new(assignment, @teacher.principal).json }
  ...
  describe "OriginalityReport" do
    ...
    it "includes the OriginalityReport in the json" do
      ...
      json = SpeedGrader::Assignment.new(assignment, test_teacher.principal).json
  ...
  describe "student anonymity" do
    ...
    let(:json) { SpeedGrader::Assignment.new(assignment, teacher.principal, avatars: true, grading_role: :moderator).json }
    let(:grader_json) { SpeedGrader::Assignment.new(assignment, ta.principal, avatars: true, grading_role: :grader).json }
    ...
    context "when the user can view student names" do
      ...
      it "optionally does not include avatars" do
        submission_1.add_comment(author: student_1, comment: "Example")
        json = SpeedGrader::Assignment.new(assignment, teacher.principal, avatars: false).json
    ...
    context "when the user cannot view student names" do
      ...
      context "when a submission has multiple versions" do
        it "uses the current submission's anonymous ID for older versions that lack one" do
          ...
          json = SpeedGrader::Assignment.new(assignment, teacher.principal, avatars: true, grading_role: :moderator).json
  ...
  describe "grader anonymity" do
    ...
    context "when the user is the final grader and cannot view other grader names" do
      let(:json) { SpeedGrader::Assignment.new(assignment, final_grader.principal, avatars: true, grading_role: :moderator).json }
    ...
    context "when the user is not the final grader and can view other grader names" do
      let(:json) { SpeedGrader::Assignment.new(assignment, teacher.principal, avatars: true, grading_role: :provisional_grader).json }
  ...
  describe "#anonymize_students?" do
    ...
    let(:speed_grader_assignment) { SpeedGrader::Assignment.new(assignment, @teacher.principal) }
  ...
  describe "#anonymous_students?" do
    ...
    context "when anonymize_students? is true" do
      ...
      it "returns true regardless of user permissions" do
        expect(speed_grader_assignment.anonymous_students?(current_principal: @teacher.principal, assignment:)).to be true
      end
    end
    ...
    context "when anonymize_students? is false" do
      ...
      it "returns false when user has manage_grades or view_all_grades permission" do
        allow(assignment.context).to receive(:grants_any_right?).with(@teacher.principal, :manage_grades, :view_all_grades).and_return(true)
        expect(speed_grader_assignment.anonymous_students?(current_principal: @teacher.principal, assignment:)).to be false
      end

      it "returns true when user has neither manage_grades nor view_all_grades permission" do
        allow(assignment.context).to receive(:grants_any_right?).with(@teacher.principal, :manage_grades, :view_all_grades).and_return(false)
        expect(speed_grader_assignment.anonymous_students?(current_principal: @teacher.principal, assignment:)).to be true
      end
    end
  end
