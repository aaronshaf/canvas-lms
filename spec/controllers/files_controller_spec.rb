...
describe FilesController do
  ...
  describe "GET 'show'" do
    ...
    context "text/html disposition" do
      let(:html_payload) { "<html><body><script>alert(1)</script></body></html>" }

      it "forces download for text/html when no files_domain is configured" do
        user_session(@teacher)
        allow(HostUrl).to receive(:has_file_host?).and_return(false)
        html_attachment = attachment_with_context(
          @course,
          uploaded_data: stub_file_data("xss.html", html_payload, "text/html")
        )
        # Without a separate files_domain, text/html served inline runs on
        # the app origin -> XSS in the viewing user's session. Force the
        # attachment branch so the browser saves rather than renders it.
        expect_any_instance_of(FilesController).to receive(:send_stored_file).with(html_attachment, inline: false)
        get "show", params: { course_id: @course.id, id: html_attachment.id, inline: 1, download: 1, verifier: html_attachment.uuid }
      end

      it "forces download for text/html on the preview path when no files_domain is configured" do
        user_session(@teacher)
        allow(HostUrl).to receive(:has_file_host?).and_return(false)
        html_attachment = attachment_with_context(
          @course,
        ...
        expect_any_instance_of(FilesController).to receive(:send_stored_file).with(html_attachment, inline: false)
        get "show", params: { course_id: @course.id, id: html_attachment.id, download: 1, verifier: html_attachment.uuid }
      end

      it "forces download for application/xhtml+xml when no files_domain is configured" do
        user_session(@teacher)
        allow(HostUrl).to receive(:has_file_host?).and_return(false)
        xhtml_attachment = attachment_with_context(
          @course,
          uploaded_data: stub_file_data("xss.xhtml", html_payload, "application/xhtml+xml")
        )
        expect_any_instance_of(FilesController).to receive(:send_stored_file).with(xhtml_attachment, inline: false)
      ...
      it "still allows inline text/html when a separate files_domain is configured" do
        ...
        # vulnerable code path doesn't execute on the app origin, so the
        # legacy inline behavior is preserved.
        allow(HostUrl).to receive(:has_file_host?).and_return(true)
        html_attachment = attachment_with_context(
          @course,
          uploaded_data: stub_file_data("page.html", "<html><body>ok</body></html>", "text/html")
        )
        expect_any_instance_of(FilesController).to receive(:send_stored_file).with(html_attachment)
        get "show", params: { course_id: @course.id, id: html_attachment.id, inline: 1, download: 1, verifier: html_attachment.uuid }
      end
    end
