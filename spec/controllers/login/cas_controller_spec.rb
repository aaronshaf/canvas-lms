...
# Multiple describes are necessary here: request specs for main tests, controller specs
# for site_admin tests which require special Account.site_admin routing via controller spec style.
describe Login::CasController, type: :request do
  def stubby(stub_response)
    ...
    def cas_client.validate_service_ticket(service_ticket)
  ...
  it "logouts with specific cas ticket" do
    ...
    post "/login/cas", params: { logoutRequest: request_text }
  ...
  it "doesn't allow deleted users to login" do
    ...
    cas_client.instance_variable_set(:@conf_options, {})
  ...
  it "accepts extra attributes" do
    account = account_with_cas(account: Account.default)
    user_with_pseudonym(active_all: true, account:, username: "testuser@example.com")
  ...
  it "scopes logins to the correct domain root account" do
    ...
    account1 = account_with_cas(account: Account.default)
  ...
  context "unknown user" do
    ...
    it "redirects when a user is authorized but not found in canvas" do
      ...
      expect_any_instance_of(Login::CasController).not_to receive(:logout_user_action)
  ...
  it "times out correctly" do
    ...
    expect(cas_client).to receive(:validate_service_ticket).and_raise(Timeout::Error)

    get "/login/cas", params: { ticket: "ST-abcd" }
    expect(response).to redirect_to(login_url)
    expect(flash[:delegated_message]).not_to be_blank
    expect(Time.now.utc - start).to be < 2
  ...
  it "does not force otp reconfiguration on succesful login" do
    ...
    get "/login/cas", params: { ticket: "ST-efgh" }
...
# Account.site_admin routing and multi-account scoping tests require controller spec style
# due to MultipleRootAccounts middleware limitations. The middleware determines the account
# from request properties (host, domain) before tests can intervene. These tests need to
# manually set controller.request.env["canvas.domain_root_account"] to test login behavior
# across different accounts or site_admin, which isn't possible in request specs.
