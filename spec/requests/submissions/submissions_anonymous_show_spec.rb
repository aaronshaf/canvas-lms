...
RSpec.describe "anonymous submission preview" do
  describe "GET /courses/:course_id/assignments/:assignment_id/anonymous_submissions/:anonymous_id" do
    ...
    it "renders the preview for the correct assignment" do
      ...
      url = "/courses/#{course.id}/assignments/#{assignment.id}" \
            "/anonymous_submissions/#{submission.anonymous_id}"
      get url, params: { preview: true }
      expect(response).to have_http_status(:ok)
      expect(response.body).to include(assignment.title)
    end

    it "renders a no submission message when the anonymous_id does not match an active submission" do
      submission = assignment.submit_homework(student, submission_type: "online_text_entry", body: "hello")
      anon_id = submission.anonymous_id
      Submission.delete_all
      user_session(teacher)
      url = "/courses/#{course.id}/assignments/#{assignment.id}" \
            "/anonymous_submissions/#{anon_id}"
      get url, params: { preview: true }
      expect(response).to have_http_status(:ok)
      expect(response.body).to include("No Submission")
    end

    it "renders the submission content for the given anonymous_id" do
      submission = assignment.submit_homework(student, submission_type: "online_text_entry", body: "hello world")
      url = "/courses/#{course.id}/assignments/#{assignment.id}" \
            "/anonymous_submissions/#{submission.anonymous_id}"
      get url, params: { preview: true }
      expect(response).to have_http_status(:ok)
    ...
    it "renders the version from submission history when version and preview params are provided" do
      ...
      url = "/courses/#{course.id}/assignments/#{assignment.id}" \
            "/anonymous_submissions/#{submission.anonymous_id}"
      get url, params: { preview: true, version: 0 }
      expect(response).to have_http_status(:ok)
      doc = Nokogiri::HTML(response.body)
      expect(doc.css("#submission_preview").text).to include("initial body")
    end

    it "renders the root submission when the given version does not match" do
      submission = assignment.submit_homework(student, submission_type: "online_text_entry", body: "pre-submission")
      Timecop.freeze(2.hours.ago) do
        submission.with_versioning(explicit: true) do
          submission.body = "initial body"
      ...
      url = "/courses/#{course.id}/assignments/#{assignment.id}" \
            "/anonymous_submissions/#{submission.anonymous_id}"
      get url, params: { preview: true, version: 99 }
      expect(response).to have_http_status(:ok)
      doc = Nokogiri::HTML(response.body)
      expect(doc.css("#submission_preview").text).to include("updated body")
    end

    it "redirects to the quiz when the assignment is a quiz" do
      quiz = course.quizzes.build(title: "Test Quiz", quiz_type: "assignment")
      quiz.workflow_state = "available"
      quiz.save!
      anon_id = quiz.assignment.submission_for_student(student).anonymous_id
      url = "/courses/#{course.id}/assignments/#{quiz.assignment.id}" \
            "/anonymous_submissions/#{anon_id}"
      get url, params: { preview: true, version: 1 }
