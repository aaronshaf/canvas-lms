...
describe('DashboardHeader', () => {
  ...
  beforeEach(() => {
    window.ENV = {
      ...
      // @ts-expect-error -- legacy ENV key not in GlobalEnv typing
  ...
  describe('as an observer', () => {
    beforeEach(() => {
      // @ts-expect-error -- test ENV only includes fields DashboardHeader reads
      window.ENV = {
    ...
    it('does not trigger page reload on mount when widget_dashboard feature flag is on but user is on classic dashboard', async () => {
      ...
      window.ENV = {
        ...
        CAN_ADD_OBSERVEE: false,
        FEATURES: {
          // @ts-expect-error -- legacy flag not yet included in typed feature union
