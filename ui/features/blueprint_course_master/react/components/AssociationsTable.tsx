...
> {
  ...
  onRemove = (courseId: string, courseName: string, focusIndex: number) => {
  ...
  renderCellText(text: string | undefined) {
    return (
  ...
  renderRemoveButton(course: Course, focusNode: {ref: (c: any) => void; index: number}) {
    return (
      <IconButton
  ...
  renderRestoreLink(course: Course, focusNode: {ref: (c: any) => void; index: number}) {
    return (
  ...
  renderRows(associations: Course[], removing = false) {
    ...
    return associations.map((course: Course & {concluded?: boolean}) => {
