...
$(document).ready(() => {
  ...
  const stop_debugging = function ($link: JQuery) {
  ...
  const load_debug_data = function ($link: JQuery, new_debug_session: boolean) {
    ...
    $.ajaxJSON(url, method, {}, (data: DebugDataResponse) => {
      if (data) {
        if (data.debugging) {
