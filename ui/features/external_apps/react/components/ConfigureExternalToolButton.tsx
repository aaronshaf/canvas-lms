...
type ConfigureExternalToolButtonProps = {
  tool: Tool
  returnFocus: () => void
  modalIsOpen?: boolean
}

type ConfigureExternalToolButtonState = {
  modalIsOpen: boolean
}
...
> {
  ...
  componentDidMount() {
    this.removeCloseListener = onLtiClosePostMessage(
      () => this.iframe,
      () => this.closeModal(),
    )
  }
    ...
    return (
      <div>
        <ToolLaunchIframe
          ...
          ref={e => {
            this.iframe = e as unknown as HTMLIFrameElement | null
          }}
        />
      </div>
    )
  ...
  iframeWidth = (): number | undefined => this.props.tool.tool_configuration?.selection_width
  iframeHeight = (): number | undefined => this.props.tool.tool_configuration?.selection_height
  ...
  headingWidth = () => {
    const width = this.iframeWidth()
    if (typeof width === 'number' && width > 50) return width - 40
    return undefined
  }
