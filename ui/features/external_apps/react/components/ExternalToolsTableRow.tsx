...
type Tool = Record<string, unknown> & {
  ...
  editor_button_settings?: (Record<string, unknown> & {enabled?: boolean}) | null
  top_navigation_settings?: {enabled?: boolean} | null
...
type ExternalToolsTableRowProps = {
  tool: Tool
  canAdd: boolean
  canEdit: boolean
  canDelete: boolean
  setFocusAbove: () => void
  rceFavoriteCount: number
  topNavFavoriteCount: number
  showLTIFavoriteToggles?: boolean
}

type ExternalAppsStoreState = {
  externalTools: Tool[]
}

type ExternalAppsStore = {
  getState: () => ExternalAppsStoreState
  setState: (state: Partial<ExternalAppsStoreState>) => void
  setAsFavorite: (
    tool: Tool,
    checked: boolean,
    type: FavoriteType,
  ...
  fetchWithDetails: (tool: Tool) => Promise<unknown>
}

const extAppStore = store as unknown as ExternalAppsStore

export default class ExternalToolsTableRow extends React.Component<ExternalToolsTableRowProps> {
  static propTypes = {
  ...
  returnFocus = (opts: {passFocusUp?: boolean} = {}) => {
    if (opts.passFocusUp) {
  ...
  handleFavoriteChange = (type: FavoriteType) => (event: unknown) => {
    const checked = (event as {target?: {checked?: unknown}} | null)?.target?.checked
    if (typeof checked === 'boolean') this.changeFavoriteLTI(checked, type)
  }

  changeFavoriteLTI = (checked: boolean, type: FavoriteType) => {
    const success = (_res: unknown) => {
      ...
      extAppStore.setState({externalTools})
    }
    ...
    extAppStore.setAsFavorite(this.props.tool, checked, type, success, error)
  }

  updateTool = (tool: {id?: string; editor_button?: unknown}) => {
    const externalTools = extAppStore.getState().externalTools
    const toolRef = externalTools.find(t => String(t.app_id) === tool.id)
    if (toolRef) {
      if (tool.editor_button && typeof tool.editor_button === 'object') {
        toolRef.editor_button_settings = tool.editor_button as Record<string, unknown> & {
          enabled?: boolean
        }
      }
      extAppStore.setState({externalTools})
  ...
  validateTool = (r: unknown, placement: string) => {
    if (placement === 'editor_button') {
      const editorButtonEnabled = (r as {editor_button?: {enabled?: unknown}} | null)?.editor_button
        ?.enabled
      if (editorButtonEnabled === false && this.props.tool.is_rce_favorite) {
        this.changeFavoriteLTI(false, 'rce')
      }
      extAppStore.fetchWithDetails(this.props.tool).then(data => {
        this.updateTool(data as {id?: string; editor_button?: unknown})
      })
    }
  }
  ...
  renderButtons = () => {
    ...
    if (tool.installed_locally && !tool.restricted_by_master_course && permsToRenderSettingsCog) {
      ...
      return (
        <td className="links text-right" style={{whiteSpace: 'nowrap'}}>
          {updateBadge}
      ...
      return (
        <td className="links text-right e-tool-table-data" style={{whiteSpace: 'nowrap'}}>
          <ExternalToolPlacementButton
            tool={tool}
            type="button"
            returnFocus={this.returnFocus}
  ...
  render() {
    ...
    const editorButtons =
      (INST.editorButtons as unknown as
        | Array<{id: string | number; on_by_default?: boolean}>
        | undefined) ?? []
            ...
            {canBeRCEFavorite(tool) ? (
              <Checkbox
                ...
                disabled={
                  ...
                  !editorButtons.find(b => b.id === tool.app_id)?.on_by_default
                }
              />
            ) : (
              I18n.t('NA')
...
function canBeRCEFavorite(tool: Tool) {
  return (
...
// see lib/lti/app_collator.rb#external_tool_definition
function canBeTopNavFavorite(tool: Tool) {
  return (
