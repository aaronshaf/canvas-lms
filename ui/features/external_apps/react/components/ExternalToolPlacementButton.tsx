...
type ExternalToolPlacementButtonProps = {
  tool: Tool
  type?: string // specify "button" if this is not a menu item
  returnFocus: () => void
  onToggleSuccess: (response: unknown, placement: string) => void
}

type ExternalToolPlacementButtonState = {
  tool: Tool
  modalIsOpen: boolean
  isRetrievingTool: boolean
}

type ExternalAppsStore = {
  fetchWithDetails: (tool: Tool) => Promise<Record<string, unknown>>
}
...
export default class ExternalToolPlacementButton extends React.Component<
  ExternalToolPlacementButtonProps,
  ExternalToolPlacementButtonState
> {
  static propTypes = {
  ...
  state: ExternalToolPlacementButtonState = {
  ...
  openModal = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    extAppStore.fetchWithDetails(this.props.tool).then(data => {
      this.setState({
        tool: {...data, ...this.props.tool} as Tool,
        isRetrievingTool: false,
      })
    })
  }
