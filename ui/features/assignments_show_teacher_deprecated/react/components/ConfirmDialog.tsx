...
export default class ConfirmDialog extends React.Component<ConfirmDialogProps> {
  ...
  closeButtonRef(elt: HTMLElement | null): void {
  ...
  renderBusyMaskBody(): React.ReactNode {
    if (this.props.busyMaskBody) return this.props.busyMaskBody()
    return <Spinner size="small" renderTitle={this.props.spinnerLabel} />
  }

  renderButton = (buttonProps: ButtonProps, index: number): React.ReactElement => {
    const defaultProps = {
      disabled: this.props.disabled,
      margin: '0 x-small 0 0' as const,
    }
