...
if (!('INST' in window)) (window as any).INST = {}
...
ready(() => {
  ...
  Array.from(comments).forEach(comment => {
    const content = (comment as HTMLElement).dataset.content || ''
...
function renderItemAssignToTray(open: boolean, returnFocusTo: any, itemProps: any) {
  createOrUpdateRoot(
...
function openSendTo(event: any, open = true) {
...
function openCopyTo(event: any, open = true) {
  if (event) event.preventDefault()
...
$(() => {
  ...
  $('#edit_assignment_form').bind('assignment_updated', (_, data: any) => {
