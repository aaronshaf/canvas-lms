...
describe('LearningObjectivesInput', () => {
  describe('rendering', () => {
    ...
    it('shows the external error prop', () => {
      render(<Wrapper error="Please provide at least one talking point" />)
    ...
    it('renders edit and delete buttons with objective text in screen reader label', () => {
      ...
      expect(
        screen.getByRole('button', {name: /Edit talking point: Critical thinking/}),
      ).toBeInTheDocument()
      expect(
        screen.getByRole('button', {name: /Delete talking point: Critical thinking/}),
      ).toBeInTheDocument()
    })
  })
  ...
  describe('add flow', () => {
    ...
    it('shows inline error and does not add when confirming blank input', async () => {
      ...
      fireEvent.click(screen.getByTestId('learning-objectives-confirm-btn'))
      await waitFor(() => {
        expect(screen.getByText('Talking point cannot be blank')).toBeInTheDocument()
      })
      expect(onChange).not.toHaveBeenCalled()
    })
    ...
    it('clears the inline error when user starts typing', async () => {
      ...
      expect(screen.queryByText('Talking point cannot be blank')).not.toBeInTheDocument()
    })
    ...
    it('pressing Enter confirms the add', () => {
      ...
      fireEvent.keyDown(screen.getByLabelText('Talking point:'), {key: 'Enter'})
  ...
  describe('edit flow', () => {
    it('shows the inline edit row pre-filled when pencil is clicked', () => {
      ...
      expect(screen.getByLabelText('Talking point:')).toHaveValue('Original text')
    })
    ...
    it('updates the objective and calls onChange on confirm edit', () => {
      ...
      fireEvent.click(screen.getByTestId('learning-objectives-edit-btn-0'))
      fireEvent.change(screen.getByLabelText('Talking point:'), {target: {value: 'New text'}})
    ...
    it('restores the original text and does not call onChange on cancel edit', () => {
      ...
      fireEvent.click(screen.getByTestId('learning-objectives-edit-btn-0'))
      fireEvent.change(screen.getByLabelText('Talking point:'), {target: {value: 'Changed'}})
    ...
    it('pressing Enter confirms the edit', () => {
      ...
      fireEvent.click(screen.getByTestId('learning-objectives-edit-btn-0'))
      fireEvent.change(screen.getByLabelText('Talking point:'), {target: {value: 'New'}})
      fireEvent.keyDown(screen.getByLabelText('Talking point:'), {key: 'Enter'})
    ...
    it('only one row is in edit mode at a time', () => {
      ...
      expect(screen.getByLabelText('Talking point:')).toHaveValue('Obj B')
