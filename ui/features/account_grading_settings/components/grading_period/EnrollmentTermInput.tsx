...
type TagType = 'active' | 'future' | 'past' | 'undated'

const groupByTagType = function (options: EnrollmentTerm[]): Record<TagType, EnrollmentTerm[]> {
  const now = new Date()
  return groupBy(options, (option): TagType => {
    const noStartDate = !isDate(option.startAt)
    const noEndDate = !isDate(option.endAt)
    const started = option.startAt ? option.startAt < now : false
    const ended = option.endAt ? option.endAt < now : false
    ...
    return 'undated'
  }) as Record<TagType, EnrollmentTerm[]>
}
...
const EnrollmentTermInput = ({
  ...
  const filteredTagsForType = (type: TagType): EnrollmentTerm[] => {
  ...
  const dismissTag = (e: React.MouseEvent<HTMLElement>, termId: string) => {
    e.stopPropagation()
    e.preventDefault()
