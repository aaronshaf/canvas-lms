...
class QuestionInspector extends React.Component<QuestionInspectorProps> {
  static defaultProps = {
    question: undefined,
    events: [],
  }
  ...
  renderQuestion(question: any) {
    const currentEventId: any = this.props.currentEventId
    const answers: any[] = []
    this.props
      .events!.filter(function (event: any) {
        return (
          event.type === K.EVT_QUESTION_ANSWERED &&
          event.data.some(function (record: any) {
      ...
      .forEach(function (event: any) {
        const records = event.data.filter(function (record: any) {
          return record.quizQuestionId === question.id
        })
  ...
  renderAnswer(record: any, index: any) {
    let answer
