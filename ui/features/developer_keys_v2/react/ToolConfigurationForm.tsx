...
type ToolConfigurationFormProps = {
  ...
  validScopes?: Record<string, unknown>
  editing: boolean
  ...
  updatePastedJson?: (value: string, pasteAtEnd: boolean) => void
  canPrettify?: boolean
}

type ToolConfigurationFormState = {
  isUrlValid: boolean
  jsonUrl: string
  showMessages: boolean
}

type ManualConfigRef = {generateToolConfiguration: () => unknown; valid: () => boolean}
type FocusableHandle = {focus?: () => void} | null
type MutableRef<T> = {current: T}

const ManualConfigurationFormUntyped = ManualConfigurationForm as unknown as React.ComponentClass<
...
> {
  isValid = true
  manualConfigRef: ManualConfigRef | null = null
  ...
  jsonRef: MutableRef<FocusableHandle> = {current: null}
  urlRef: MutableRef<FocusableHandle> = {current: null}

  validateUrlField = (fieldValue: string, fieldRef: MutableRef<FocusableHandle>) => {
  ...
  valid = () => {
    ...
    if (this.isManual()) {
      ...
      this.validateUrlField(this.state.jsonUrl, this.urlRef)
    }
  ...
  updatePastedJson = (e: {target: {value: string; selectionEnd: number}}) => {
    this.props.updatePastedJson?.(e.target.value, e.target.selectionEnd === e.target.value.length)
  }

  handleJsonChange = (e: {target: {value: string; selectionEnd: number}}) => {
    try {
  ...
  handleToolConfigUrlChange = (e: {target: {value: string}}) => {
    const value = e.target.value
    this.setState({jsonUrl: value})
    this.validateUrlField(value, this.urlRef)
    this.props.updateToolConfigurationUrl(value)
  }

  handleConfigTypeChange = (_e: unknown, option: {value?: string | number}) => {
    if (option.value !== 'manual' && option.value !== 'json' && option.value !== 'url') return
  ...
  setManualConfigRef = (node: unknown) => {
    this.manualConfigRef = node as ManualConfigRef | null
  }
      ...
      <Grid.Row>
        <Grid.Col>
          <TextArea
            ...
            textareaRef={ref => {
              this.jsonRef.current = ref as FocusableHandle
            }}
            maxHeight="20rem"
  ...
  urlConfigurationInput = () => (
    <TextInput
      ...
      inputRef={ref => {
        this.urlRef.current = ref as FocusableHandle
      }}
  ...
  manualConfigurationInput = (visible: boolean) => (
    <div style={{display: visible ? undefined : 'none'}}>
      <ManualConfigurationFormUntyped
