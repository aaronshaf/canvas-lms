...
const DeveloperKeyModalTriggerUntyped = DeveloperKeyModalTrigger as unknown as React.ComponentType<
...
type Header = {
  ...
  sortable: boolean
  sortText?: string
  sortValue?: (key: DeveloperKeyData) => unknown
}
...
const createSetFocusCallback =
  ({
    ...
    developerKeysList: DeveloperKeyData[]
    developerKeyRef: (key: DeveloperKeyData) => DeveloperKeyRowHandle | undefined
    srMsg: string
    handleRef: (ref: DeveloperKeyRowHandle | undefined) => void
  }) =>
  (developerKeys: DeveloperKeyData[]) => {
    $.screenReaderFlashMessageExclusive(srMsg)
    const developerKey = developerKeysList
      .concat(developerKeys)
      .reverse()
      .find((key: DeveloperKeyData) => {
        const keyRef = developerKeyRef(key)
        return keyRef && !(keyRef.isDisabled?.() ?? false)
      })
...
class AdminTable extends React.Component<AdminTableProps, AdminTableState> {
  ...
  addDevKeyButton: HTMLElement | null = null
  ...
  headers = {
    ...
    'keystable-owneremail': {
      ...
      sortValue: (key: DeveloperKeyData) => key.email,
    },
    'keystable-details': {
    ...
    'keystable-stats': {
      ...
      sortValue: (key: DeveloperKeyData) => key.access_token_count,
    },
    'keystable-type': {
      id: 'keystable-type',
      text: I18n.t('Type'),
    ...
    'keystable-state': {
      ...
      sortValue: (key: DeveloperKeyData) => key.developer_key_account_binding?.workflow_state,
    },
    'keystable-actions': {
  ...
  renderHeader = () => {
    ...
    return (
      <Table.Row>
        {(Object.values(this.headers) as Header[]).map(header => (
          <Table.ColHeader
            key={header.id}
            id={header.id}
            {...(header.sortable && {
  ...
  sortedDeveloperKeys = () => {
    ...
    return this.filteredDeveloperKeys().sort((a, b) => {
      // @ts-expect-error TS migration: state-driven key doesn't narrow to headers keys.
      const header = this.headers[sortBy]
  ...
  onDelete = (developerKeyId: string) => {
  ...
  setAddKeyButtonRef = (node: HTMLElement | null) => {
    ...
    return (
      <div>
        <Flex justifyItems="space-between" margin="0" wrap="wrap">
          ...
          />
          <DeveloperKeyModalTriggerUntyped
        ...
        >
          ...
          <Table.Body>
            {developerKeys.map(developerKey => (
              <React.Fragment key={developerKey.id}>
                <DeveloperKeyUntyped
