...
export default class Placements extends React.Component<PlacementsProps, PlacementsState> {
  ...
  private placementRefs: Record<string, Placement> = {}
  ...
  valid = (): boolean => Object.values(this.placementRefs).every(p => p.valid())

  placements(obj: PlacementData[]): string[] {
    return obj.map(o => o.placement)
  }

  placementDisplayName(p: string): string {
  ...
  setPlacementRef = (placement: string) => (node: Placement | null) => {
  ...
  newPlacements(placements: string[]): PlacementData[] {
