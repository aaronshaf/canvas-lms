...
const validationMessage: {url: {text: string; type: 'error'}[]} = {
  url: [{text: I18n.t('Please enter a valid URL (e.g. https://example.com)'), type: 'error'}],
}

type PlacementConfig = Record<string, unknown> & {
  message_type?: string
  name?: string
...
export default class Placement extends React.Component<PlacementProps, PlacementState> {
  ...
  targetLinkUriRef: MutableRef<FocusableHandle> = {current: null}
  iconUrlRef: MutableRef<FocusableHandle> = {current: null}
  ...
  validateUrlField = (
    ...
    fieldStateKey: 'isTargetLinkUriValid' | 'isIconUrlValid',
    fieldRef: MutableRef<FocusableHandle>,
  ) => {
    if (fieldValue && !URL.canParse(fieldValue)) {
      if (fieldStateKey === 'isTargetLinkUriValid') {
        this.setState({isTargetLinkUriValid: false})
      } else {
        this.setState({isIconUrlValid: false})
      }
      if (this.isValid) {
      ...
      if (fieldStateKey === 'isTargetLinkUriValid') {
        this.setState({isTargetLinkUriValid: true})
      } else {
        this.setState({isIconUrlValid: true})
      }
    }
  }
  ...
  setOrDeletePlacementField = (key: string, value: unknown) => {
    this.setState(state => {
  ...
  handleMessageTypeChange = (_: unknown, value: string) =>
    this.setState(state => ({placement: {...state.placement, message_type: value}}))

  handleIconUrlChange = (e: {target: {value: string}}) => {
    const value = e.target.value
  ...
  handleTextChange = (e: {target: {value: string}}) => {
    const value = e.target.value
    this.setState(state => ({placement: {...state.placement, text: value}}))
  }

  handleHeightChange = (e: {target: {value: string; name: string}}) => {
    const value = e.target.value
  ...
  handleWidthChange = (e: {target: {value: string; name: string}}) => {
    const value = e.target.value
    const numVal = Number.parseInt(value, 10)
            ...
            >
              ...
              >
                <TextInput
                  ...
                  inputRef={(ref: unknown) => {
                    this.targetLinkUriRef.current = ref as FocusableHandle
                  }}
              ...
              >
                <TextInput
                  ...
                  inputRef={(ref: unknown) => {
                    this.iconUrlRef.current = ref as FocusableHandle
                  }}
