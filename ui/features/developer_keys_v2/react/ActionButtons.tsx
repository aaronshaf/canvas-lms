...
type DeveloperKeyActionButtonsProps = {
  dispatch: (action: unknown) => unknown
  makeVisibleDeveloperKey: (developerKey: DeveloperKey) => unknown
  makeInvisibleDeveloperKey: (developerKey: DeveloperKey) => unknown
  deleteDeveloperKey: (developerKey: DeveloperKey) => unknown
  editDeveloperKey: (developerKey: DeveloperKey) => unknown
  developerKeysModalOpen: (mode: string) => unknown
  ltiKeysSetLtiKey: (isLti: boolean) => unknown
  contextId: string
...
class DeveloperKeyActionButtons extends React.Component<DeveloperKeyActionButtonsProps> {
  deleteLink: HTMLElement | null = null

  makeVisibleLinkHandler = (event: {preventDefault: () => void}) => {
    const {dispatch, makeVisibleDeveloperKey, developerKey} = this.props
    event.preventDefault()
    dispatch(makeVisibleDeveloperKey(developerKey))
  }

  makeInvisibleLinkHandler = (event: {preventDefault: () => void}) => {
  ...
  deleteLinkHandler = async (event: {preventDefault: () => void}) => {
  ...
  editLinkHandler = (event: {preventDefault: () => void}) => {
    const {
      dispatch,
      editDeveloperKey,
  ...
  refDeleteLink = (link: HTMLElement | null) => {
        ...
        <Tooltip renderTip={tooltipText}>
          {
            <IconButton
              ...
              elementRef={this.refDeleteLink as unknown as (node: unknown) => void}
              disabled={ENV.devKeysReadOnly}
            >
              <IconTrashLine />
            </IconButton>
