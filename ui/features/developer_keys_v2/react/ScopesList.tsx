/*
 * Copyright (C) 2018 - present Instructure, Inc.
 *
 * This file is part of Canvas.
 *
 * Canvas is free software: you can redistribute it and/or modify it under
 * the terms of the GNU Affero General Public License as published by the Free
 * Software Foundation, version 3 of the License.
 *
 * Canvas is distributed in the hope that it will be useful, but WITHOUT ANY
 * WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR
 * A PARTICULAR PURPOSE. See the GNU Affero General Public License for more
 * details.
 *
 * You should have received a copy of the GNU Affero General Public License along
 * with this program. If not, see <http://www.gnu.org/licenses/>.
 */
import {useScope as createI18nScope} from '@canvas/i18n'
import LazyLoad from 'react-lazy-load'
import PropTypes from 'prop-types'
import React from 'react'
import {View} from '@instructure/ui-view'
import {Flex} from '@instructure/ui-flex'
import {ScreenReaderContent, PresentationContent} from '@instructure/ui-a11y-content'
import {Text} from '@instructure/ui-text'
import {Checkbox} from '@instructure/ui-checkbox'
import ScopesGroup from './ScopesGroup'
import ScopesMethod from './ScopesMethod'

const I18n = createI18nScope('react_developer_keys')

type ScopeItem = {
  resource?: string
  scope: string
  verb: string
}

type AvailableScopes = Record<string, unknown>

type ScopesListProps = {
  dispatch: Function
  listDeveloperKeyScopesSet: Function
  availableScopes: AvailableScopes
  filter: string
  selectedScopes?: string[]
}

type ScopeGroupMap = Record<string, ScopeItem[]>

type ScopesListState = {
  formattedScopesArray: ScopeGroupMap[]
  availableScopes: ScopeGroupMap[]
  selectedScopes: string[]
  readOnlySelected: boolean
}

export default class ScopesList extends React.Component<ScopesListProps, ScopesListState> {
  constructor(props: ScopesListProps) {
    super(props)
    const formattedScopesArray = Object.keys(this.props.availableScopes).map(k => {
      const value = this.props.availableScopes[k]
      const scopes = Array.isArray(value) ? (value as ScopeItem[]) : ([value] as ScopeItem[])
      return {[k]: scopes}
    })

    this.state = {
      formattedScopesArray,
      availableScopes: formattedScopesArray.slice(0, 8), // Only load 8 groups on initial render
      selectedScopes: this.props.selectedScopes ?? [],
      readOnlySelected: this.onlySelectGet(
        this.uniqueSelectedScopes(this.props.selectedScopes ?? []),
      ),
    }
  }

  componentDidMount() {
    this.delayedRender()
  }

  onlySelectGet(selectedScopes: string[]) {
    const allAvailableGetScopes = this.availableGetScopes()
    if (selectedScopes.length !== allAvailableGetScopes.length) {
      return false
    }
    const nonGetScopes = selectedScopes.filter(s => !allAvailableGetScopes.includes(s))
    if (nonGetScopes.length > 0) {
      return false
    }
    return true
  }

  setSelectedScopes = (scope: string[]) => {
    const selectedScopes = this.uniqueSelectedScopes(scope)
    this.setState({
      selectedScopes,
      readOnlySelected: this.onlySelectGet(selectedScopes),
    })
    this.props.dispatch(this.props.listDeveloperKeyScopesSet(selectedScopes))
  }

  uniqueSelectedScopes(selectedScopes: string[]) {
    return [...new Set(selectedScopes)]
  }

  delayedRender = () => {
    // Load the rest of the groups once the modal is open
    setTimeout(() => {
      this.setState(oldState => ({availableScopes: oldState.formattedScopesArray.slice()}))
    }, 0)
  }

  handleReadOnlySelected = (event: {currentTarget: {checked: boolean}}) => {
    let newScopes: string[] = []
    if (event.currentTarget.checked) {
      newScopes = this.availableGetScopes()
    } else {
      newScopes = []
    }

    this.setState({
      selectedScopes: newScopes,
      readOnlySelected: event.currentTarget.checked,
    })

    this.props.dispatch(this.props.listDeveloperKeyScopesSet(newScopes))
  }

  noFilter() {
    return this.props.filter === '' || !this.props.filter
  }

  availableGetScopes() {
    return this.allScopes(this.props.availableScopes)
      .filter(s => s.verb === 'GET')
      .map(s => s.scope)
  }

  allScopes(availableScopes: AvailableScopes) {
    return Object.values(availableScopes).reduce<ScopeItem[]>((accumulator, currentValue) => {
      const scopes = Array.isArray(currentValue)
        ? (currentValue as ScopeItem[])
        : ([currentValue] as ScopeItem[])
      return accumulator.concat(scopes)
    }, [])
  }

  render() {
    return (
      <div data-automation="scopes-list">
        <View borderRadius="small" display="block" borderWidth="small">
          <Flex height="564px" width="100%" as="div" direction="column">
            <Flex.Item padding="none" textAlign="start">
              <View padding="small" display="block" borderWidth="none none medium none">
                <Flex>
                  <Flex.Item>
                    <Checkbox
                      label={
                        <ScreenReaderContent>
                          {this.state.readOnlySelected
                            ? I18n.t('Disable all read only scopes.')
                            : I18n.t('Enable all read only scopes.')}
                        </ScreenReaderContent>
                      }
                      onChange={this.handleReadOnlySelected}
                      checked={this.state.readOnlySelected}
                      inline={true}
                    />
                    <PresentationContent>
                      <Text size="medium" weight="bold">
                        {I18n.t('Read only')}
                      </Text>
                      <ScopesMethod method="get" margin="none small none small" />
                    </PresentationContent>
                  </Flex.Item>
                </Flex>
              </View>
            </Flex.Item>
            <Flex.Item shouldGrow={true} shouldShrink={true}>
              {this.state.availableScopes.map(scopeGroup => {
                return Object.keys(scopeGroup).reduce((result, key) => {
                  const groupMatchesFilter =
                    this.noFilter() || key.toLowerCase().includes(this.props.filter.toLowerCase())
                  const scopeMatchesFilter = scopeGroup[key].some(s =>
                    s.scope.toLowerCase().includes(this.props.filter.toLowerCase()),
                  )
                  const filteredScopes =
                    this.noFilter() || groupMatchesFilter
                      ? scopeGroup[key]
                      : scopeGroup[key].filter(s =>
                          s.scope.toLowerCase().includes(this.props.filter.toLowerCase()),
                        )
                  const expanded = !this.noFilter() && scopeMatchesFilter
                  if (groupMatchesFilter || filteredScopes.length > 0) {
                    result.push(
                      <LazyLoad
                        offset={1000}
                        height={50}
                        key={`${key}-scope-group-${expanded ? 'expanded' : ''}`}
                      >
                        <ScopesGroup
                          scopes={filteredScopes}
                          name={key}
                          expanded={expanded}
                          selectedScopes={this.state.selectedScopes}
                          setSelectedScopes={this.setSelectedScopes}
                        />
                      </LazyLoad>,
                    )
                  }
                  return result
                }, [] as JSX.Element[])
              })}
            </Flex.Item>
          </Flex>
        </View>
      </div>
    )
  }
}

// @ts-expect-error TS migration: react class static propTypes not typed.
ScopesList.propTypes = {
  dispatch: PropTypes.func.isRequired,
  listDeveloperKeyScopesSet: PropTypes.func.isRequired,
  availableScopes: PropTypes.objectOf(
    PropTypes.arrayOf(
      PropTypes.shape({
        resource: PropTypes.string,
        scope: PropTypes.string,
      }),
    ),
  ).isRequired,
  filter: PropTypes.string.isRequired,
  selectedScopes: PropTypes.arrayOf(PropTypes.string),
}

// @ts-expect-error TS migration: react class static defaultProps not typed.
ScopesList.defaultProps = {
  selectedScopes: [],
}
