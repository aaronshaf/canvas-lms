...
type ScopeItem = {scope: string; verb: string}

type ScopesGroupProps = {
...
type ScopesGroupState = {groupChecked: boolean}
...
export default class ScopesGroup extends React.Component<ScopesGroupProps, ScopesGroupState> {
  constructor(props: ScopesGroupProps) {
    super(props)
    this.state = {groupChecked: this.allScopesAreSelected(props)}
  }
  ...
  shouldComponentUpdate(nextProps: ScopesGroupProps, _nextState: ScopesGroupState) {
    ...
    const selectedDiff = nextProps.selectedScopes
      .filter((scope: string) => !this.props.selectedScopes.includes(scope))
      .concat(
        this.props.selectedScopes.filter(
          (scope: string) => !nextProps.selectedScopes.includes(scope),
        ),
      )

    // Rerender if a scope owned by this group changed
    if (
      this.props.scopes.filter((scope: ScopeItem) => selectedDiff.includes(scope.scope)).length > 0
    ) {
      return true
    }
    return false
  ...
  selectedMethods() {
    const methodSet = this.props.scopes.reduce((result: Set<string>, scope: ScopeItem) => {
    ...
    return (
      <span>
        {[...methodSet].sort().map((method: string) => (
          <ScopesMethod
            method={method}
  ...
  allScopesInGroup() {
    return this.props.scopes.map((s: ScopeItem) => s.scope)
  }

  handleGroupChange = (event: {currentTarget: {checked: boolean}}) => {
  ...
  handleSingleChange = (event: {currentTarget: {checked: boolean; value: string}}) => {
