...
interface AssignmentGroupModuleNavProps {
  [key: string]: any
}

export default function AssignmentGroupModuleNav({assignment}: AssignmentGroupModuleNavProps) {
      ...
      {modules.slice(0, MAX_DISPLAYED_MODULES).map((module: any) => (
        <Flex.Item key={module.id} overflowY="visible">
          <Link
            ...
            href={env.moduleUrl}
            isWithinText={false}
            // @ts-expect-error - themeOverride type mismatch
