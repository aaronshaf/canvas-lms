...
// Legacy client-side variant of the Rubrics index page. Used when the
// `rubrics_page_pagination` feature flag is OFF: all rubrics are fetched up
// front via useAllPages, then bucketed by workflow state, filtered, and
// sorted in the browser. When the flag is on, ViewRubricsPaginated drives
// the same UI off of the server-paginated query.
...
const sortRubrics = (
  ...
  if (!sortedColumn) return rubrics
  const sorted = [...rubrics].sort((a, b) => {
    switch (sortedColumn) {
      case 'Title':
      ...
      case 'LocationUsed':
        return (a.hasRubricAssociations ? 0 : 1) - (b.hasRubricAssociations ? 0 : 1)
      default:
        return 0
    }
  })
...
export const ViewRubricsLegacy = ({
  ...
  const [sortedColumn, setSortedColumn] = useState<RubricSortColumn | undefined>(undefined)
  const [sortDirection, setSortDirection] = useState<'ascending' | 'descending'>('ascending')

  const handleSort = (columnId: RubricSortColumn) => {
  ...
  >({
    ...
    queryFn: async ({pageParam}) =>
      fetchQuery(queryVariables, {first: RUBRICS_PER_PAGE, after: pageParam as string | null}),
    refetchOnMount: true,
    getNextPageParam,
  ...
  const filterByQuery = (list: Rubric[]) =>
    searchQuery.trim() !== ''
      ? list.filter(rubric => rubric.title.toLowerCase().includes(searchQuery.toLowerCase()))
      : list

  const filteredActiveRubrics = sortRubrics(
    filterByQuery(activeRubrics),
    sortedColumn,
    sortDirection,
  )
  const filteredArchivedRubrics = sortRubrics(
    filterByQuery(archivedRubrics),
    sortedColumn,
    sortDirection,
  )
