...
class DashboardCardAction extends React.Component {
  ...
  render() {
    // @ts-expect-error -- legacy untyped React component
    const {path, linkClass, screenReaderLabel, iconClass, unreadCount} = this.props
    return (
      <a
        href={path}
        className={classnames('ic-DashboardCard__action', linkClass)}
        title={screenReaderLabel}
      >
        {screenReaderLabel ? <span className="screenreader-only">{screenReaderLabel}</span> : null}
