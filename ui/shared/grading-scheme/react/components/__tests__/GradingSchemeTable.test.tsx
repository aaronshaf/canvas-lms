...
describe('GradingSchemeTable', () => {
  ...
  const renderGradingSchemeTable = (props: Partial<GradingSchemeTableProps> = {}) => {
    const editGradingScheme = vi.fn()
    const openGradingScheme = vi.fn()
    const viewUsedLocations = vi.fn()
    const openDuplicateModal = vi.fn()
    const openDeleteModal = vi.fn()
    const archiveOrUnarchiveScheme = vi.fn()
    const funcs = render(
      <GradingSchemeTable
  ...
  it('should display the next set of grading scheme cards when clicking on a pagination button', async () => {
  ...
  it('should sort the schemes in reverse order when clicking the button to reverse the sorting', async () => {
    const {getByTestId, queryByTestId} = renderGradingSchemeTable({
      gradingSchemeCards: ExtraGradingSchemeCards,
    })
    expect(queryByTestId('grading-scheme-row-9')).not.toBeInTheDocument()
    ...
    await waitFor(() => expect(getByTestId('grading-scheme-row-9')).toBeInTheDocument())
  })
