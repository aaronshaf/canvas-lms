...
export default function EarlyAccessModal({isOpen, onAccept, onCancel}: EarlyAccessModalProps) {
  ...
  const handleAccept = async () => {
    try {
      ...
      showFlashAlert({
        message: t('An error occurred accepting the Early Access Program terms'),
        err: error as Error,
        type: 'error',
      })
    }
  ...
  return (
    <CanvasModal
      label={t('Early Access Program Terms and Conditions')}
      onDismiss={onCancel}
      open={isOpen}
      size="fullscreen"
      footer={null}
    ...
    >
      <View as="div" height="100%" overflowY="auto" padding="x-small medium">
        ...
        <Text as="p" variant="descriptionSection">
          {t("Here's a quick overview of what to expect:")}
        </Text>

        <Text as="p">
          <Text variant="contentImportant">{t('Access & Use: ')}</Text>
          {t(
        ...
        <Text as="p">
          <Text variant="contentImportant">{t('No Warranties / Limited Liability: ')}</Text>
          {t(
        ...
        <Text as="p">
          <Text variant="contentImportant">{t('Your Feedback Matters: ')}</Text>
          {t(
        ...
        <Text as="p">
          <Text variant="contentImportant">{t('Your Content: ')}</Text>
          {t(
        ...
        <Text as="p">
          <Text variant="contentImportant">{t('Proprietary Rights: ')}</Text>
          {t(
        ...
        <Text as="p">
          <Text variant="contentImportant">{t('Responsibilities: ')}</Text>
          {t(
        ...
        <Text as="p">
          <Text variant="contentImportant">{t('Confidentiality: ')}</Text>
          {t('Please keep what you see in this program confidential.')}
        </Text>

        <Text as="p">
          <Text variant="contentImportant">{t('Termination & Support: ')}</Text>
          {t(
        ...
        <Text as="p">
          {t("Thanks for partnering with us, and we're looking forward to hearing what you think.")}
        </Text>

        <View as="div" margin="large 0" textAlign="center">
          <Text as="p" variant="contentImportant">
            {t('Do you accept the Early Access Program Terms and Conditions?')}
          </Text>
