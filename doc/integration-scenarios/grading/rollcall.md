...
**Scenario RC-1.6 — Rollcall grade passback clears a teacher's excused status on an attendance submission**
- **GUID:** `2e9a7d46`
...
**Scenario RC-1.7 — Rollcall auto-creates a new attendance assignment after the original is deleted**
- **GUID:** `7e4c1d90`
...
**Scenario RC-1.8 — Changing the attendance assignment's point value preserves the raw score instead of rescaling it**
- **GUID:** `d3b9f462`
...
**Scenario RC-1.9 — Setting "exclude from final grade" in Rollcall sets the Canvas assignment omit flag**
- **GUID:** `a1f5e830`
...
**Scenario RC-1.10a — A section-limited TA can pass back an attendance grade for a student in their own section**
- **GUID:** `3a7f1e62`
- **Reason:** Positive control — without it, the RC-1.10b rejection could pass for the wrong reason (a TA who cannot grade at all). This proves the section-limited TA grades normally inside their section.
- **Note:** Canvas scopes a section-limited grader (`limit_privileges_to_course_section`) to their own section via `get_user_considering_section` (`SubmissionsApiController#update` → `Course#students_visible_to`). RC-1.10a and RC-1.10b together pin that the section limit — not a broken grader — is what rejects the out-of-section passback.
```
...
**Scenario RC-1.10b — A section-limited TA cannot pass back an attendance grade for a student outside their section**
- **GUID:** `8c6d4f15`
