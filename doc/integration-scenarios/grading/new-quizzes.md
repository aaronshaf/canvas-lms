...
- **GUID:** `5a9c3e72`
- **Reason:** Students' identities are exposed to graders during anonymous survey grading if the Canvas submissions API leaks the student's name or user identity in the submission records.
```
...
Then no submission record exposes the student's name
```
...
**Scenario NQ-1.15 — Anonymous NQ graded survey is recorded as anonymous on the Canvas assignment**
- **GUID:** `d3f9e27c`
- **Reason:** Teachers can identify anonymous survey respondents through the gradebook if the Canvas assignment does not record the survey as anonymous, exposing which students have submitted.
```
Given a New Quizzes anonymous graded survey is published in a course
When the teacher retrieves the assignment details for the survey
Then the assignment details report the survey as anonymous
```

**Scenario NQ-1.16 — Duplicated NQ quiz assignment is created in unpublished state**
- **GUID:** `b6e4a10d`
...
**Scenario NQ-1.17 — NQ graded survey type preserved during content import**
- **GUID:** `c8a15e4d`
