...
| preferred_presentation                                                            | "iframe" &#124; "window"                                                                                 | no       | **Standard LTI 1.3 field.** Indicates the preferred option to open the link. A platform may ignore this if the placement only supports a single modality.                                                                                                                                                                                                                                       |
| iframe                                                                            | JSON object with `width` and `height` (integers)                                                         | no       | **Standard LTI 1.3 field.** Gives a hint about the preferred dimensions of the iframe in pixels. Example: `{"width": 1024, "height": 800}`. Canvas will use these values instead of the Canvas-specific extension fields if present.                                                                                                                                                            |
| window                                                                            | JSON object with optional `target_name` (string), `width` and `height` (integers), `window_features` (string) | no       | **Standard LTI 1.3 field.** Indicates how to open the resource in a new window/tab. Example: `{"target_name": "lti_launch", "width": 1200, "height": 900, "window_features": "menubar=yes"}`. The `window_features` is a comma-separated list as per [window.open()](https://developer.mozilla.org/en-US/docs/Web/API/Window/open). Canvas will use these values if `preferred_presentation` is `"window"`. |
...
| ht<span>tps://</span>canvas.instructure.com/lti/launch_height                     | string &#124; number                                                                                     | no       | **Canvas extension field.** Specifies the height of the iframe the tool will be embedded in. **Note:** The standard `iframe` and `window` fields are now preferred; this field is maintained for backwards compatibility.                                                                                                                                                                       |
| ht<span>tps://</span>canvas.instructure.com/lti/launch_width                      | string &#124; number                                                                                     | no       | **Canvas extension field.** Specifies the width of the iframe the tool will be embedded in. **Note:** The standard `iframe` and `window` fields are now preferred; this field is maintained for backwards compatibility.                                                                                                                                                                        |
...
##### Presentation Properties Priority

When determining placement dimensions, Canvas uses the following priority order:

1. If `preferred_presentation` is `"window"` and `window` object exists, Canvas uses `window.width` and `window.height`
2. Otherwise, if `iframe` object exists, Canvas uses `iframe.width` and `iframe.height`
3. Otherwise, if `window` object exists (without `preferred_presentation`), Canvas uses `window.width` and `window.height`
4. Otherwise, Canvas falls back to the Canvas extension fields (`https://canvas.instructure.com/lti/launch_width` and `https://canvas.instructure.com/lti/launch_height`)

**Best Practice:** Use the standard `preferred_presentation`, `iframe`, and `window` fields for new integrations. The Canvas extension fields are maintained for backwards compatibility with existing tools.
