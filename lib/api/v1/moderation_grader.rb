...
module Api::V1::ModerationGrader
  ...
  def moderation_graders_json(assignment, current_principal, session)
    ...
    if assignment.can_view_other_grader_identities?(current_principal)
      ...
      api_json(graders, current_principal, session, only: %w[id user_id]).tap do |hash|
      ...
      api_json(provisional_graders, current_principal, session, only: %w[id anonymous_id])
