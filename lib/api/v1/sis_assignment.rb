...
module Api::V1::SisAssignment
  ...
  def sis_assignments_json(assignments, includes: {}, current_principal: nil)
    assignments.map { |a| sis_assignment_json(a, includes:, current_principal:) }
  end

  private

  def sis_assignment_json(assignment, includes: {}, current_principal: nil)
    ...
    json[:user_overrides] = assignment_user_overrides_json(assignment, current_principal:) if includes[:student_overrides]
    json
  end
  ...
  def assignment_user_overrides_json(assignment, current_principal: nil)
    overrides = active_assignment_overrides_for(assignment)
    raise UnloadedAssociationError if overrides.nil?

    overrides.filter_map { |o| assignment_user_override_json(o, current_principal:) }
  end

  def assignment_user_override_json(override, current_principal: nil)
    ...
    assignment_override_students_json = override.assignment_override_students.map do |student_override|
      ...
      if student_override.association(:user).loaded?
        pseudonym = SisPseudonym.for(student_override.user, override.assignment.context, current_user: current_principal)
