...
module Api::V1::AccountCalendar
  ...
  def account_calendars_json(accounts, current_principal, session, include: [])
    accounts.map { |account| account_calendar_json(account, current_principal, session, include:) }
  end

  def account_calendar_json(account, current_principal, session, include: [])
    json = api_json(account, current_principal, session, only: ACCOUNT_ATTRIBUTES)
    ...
    json["can_create_calendar_events"] = account.respond_to?(:calendar_events) && CalendarEvent.new.tap { |e| e.context = account }.grants_right?(current_principal, session, :create)
