...
module Api::V1::QuizSubmission
  ...
  def quiz_submission_json(qs, quiz, current_principal, session, context = nil)
    context ||= quiz.context

    hash = api_json(qs, current_principal, session, {
  ...
  def quiz_submissions_json(quiz_submissions, quiz, current_principal, session, context, includes, params)
    hash = {}
    ...
    hash[:quiz_submissions] = [quiz_submissions].flatten.map do |qs|
      quiz_submission_json(qs, quiz, current_principal, session, context)
    end
    ...
    if includes.include?("submission")
      ...
      hash[:submissions] = with_submissions.map do |qs|
        submission_json(qs.submission, quiz.assignment, current_principal, session, context, includes, params)
      end
    end

    if includes.include?("quiz")
      hash[:quizzes] = [
        quiz_json(quiz, context, current_principal, session)
      ]
    end

    if includes.include?("user")
      hash[:users] = quiz_submissions.map do |qs|
        user_json(qs.user, current_principal, session, ["avatar_url"], context, nil)
      end
    end
