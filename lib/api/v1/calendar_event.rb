...
module Api::V1::CalendarEvent
  ...
  def event_json(event, current_principal, session, options = {})
    if event.is_a?(::CalendarEvent)
      calendar_event_json(event, current_principal, session, options)
    elsif event.is_a?(::SubAssignment)
      sub_assignment_event_json(event, current_principal, session, options)
    else
      assignment_event_json(event, current_principal, session, options)
    end
  end

  def calendar_event_json(event, current_principal, session, options = {})
    ...
    request_shard = options[:request_shard] || @request_shard || current_principal.shard
    if event.effective_context_code
      ...
      if appointment_group && include_child_events
        common_context_codes = common_ag_context_codes(appointment_group, current_principal, event, options[:for_scheduler])
    ...
    hash["context_name"] = context.try(:nickname_for, current_principal)
    ...
    hash["hidden"] = current_principal ? event.hidden? : false

    if include.include?("participants")
      case event.context_type
      when "User"
        hash["user"] = user_json(event.context, current_principal, session)
      when "Group"
        hash["group"] = group_json(event.context, current_principal, session, include: ["users"])
      end
    end
    if appointment_group
      ...
      hash["can_manage_appointment_group"] = appointment_group.grants_right?(current_principal, session, :manage)
    ...
    if event.context_type == "AppointmentGroup"
      if context.grants_right?(current_principal, session, :reserve)
        participant = context.participant_for(current_principal)
    ...
    if include_child_events && hash["child_events_count"] > 0
      can_read_child_events = event.grants_right?(current_principal, session, :read_child_events)
      if can_read_child_events || hash["reserved"]
        ...
        if events.first.context.is_a?(User) && user_json_is_admin?(@context, current_principal)
          user_json_preloads(events.map(&:context))
        end
        can_manage = event.grants_right?(current_principal, session, :manage)
    ...
    if include.include?("web_conference") &&
       ...
       event.web_conference.grants_right?(current_principal, session, :read) &&
       web_conference_plugin_enabled?(event.web_conference)
      hash["web_conference"] = api_conference_json(event.web_conference, current_principal, session)
    end

    if options.key?(:url_override) ? options[:url_override] || hash["own_reservation"] : event.grants_right?(current_principal, session, :read)
    ...
    if duplicates
      hash["duplicates"] = duplicates.map { |dupe| { "calendar_event" => calendar_event_json(dupe, current_principal, session, options) } }
    end
  ...
  def assignment_event_json(assignment, current_principal, session, options = {})
    excludes = options[:excludes] || []
    ...
    hash = api_json(assignment, current_principal, session, only: target_fields)
    ...
    if excludes.include?("assignment")
      ...
      hash["assignment"] = assignment_json(assignment, current_principal, session, override_dates: false, submission: options[:submission])
    ...
    hash["context_name"] = assignment.context.try(:nickname_for, current_principal)
    ...
    if assignment.applied_overrides.present?
      all_overrides = assignment.applied_overrides.map { |o| assignment_override_json(o, current_principal:) }
  ...
  def sub_assignment_event_json(sub_assignment, current_principal, session, options = {})
    excludes = options[:excludes] || []
    parent_assignment = sub_assignment.parent_assignment

    target_fields = %w[created_at updated_at title all_day all_day_date workflow_state submission_types]
    target_fields << "description" unless excludes.include?("description")
    parent_assignment_hash = assignment_json(parent_assignment, current_principal, session, override_dates: false, submission: options[:submission])
    hash = api_json(sub_assignment, current_principal, session, only: target_fields)
    ...
    hash["sub_assignment"] = assignment_json(sub_assignment, current_principal, session, override_dates: false, submission: options[:submission])
    ...
    hash["context_name"] = sub_assignment.context.try(:nickname_for, current_principal)
    ...
    if sub_assignment.applied_overrides.present?
      hash["sub_assignment_overrides"] = sub_assignment.applied_overrides.map { |o| assignment_override_json(o, current_principal:) }
    end
    hash["important_dates"] = sub_assignment.important_dates
    hash
  end

  def appointment_group_json(group, current_principal, session, options = {})
    orig_context = @context
    @context = group.contexts_for_user(current_principal.user).first
    ...
    if include.include?("reserved_times")
      hash["reserved_times"] = group.reservations_for(current_principal).map do |event|
        {
          id: event.id,
    ...
    request_shard = @request_shard || current_principal.shard
    hash["context_codes"] = request_shard.activate { group.contexts.map(&:asset_string) }

    if include.include?("all_context_codes") && group.grants_right?(current_principal, session, :manage)
      hash["all_context_codes"] = request_shard.activate { group.contexts.map(&:asset_string) }
    end
    hash["requiring_action"] = group.requiring_action?(current_principal)
    if group.new_appointments.present?
      hash["new_appointments"] = group.new_appointments.map { |event| calendar_event_json(event, current_principal, session, skip_details: true, appointment_group_id: group.id) }
    end
    if include.include?("appointments")
      ...
      if include.include?("child_events")
        ...
        user_json_preloads(all_child_events.map(&:context)) if !all_child_events.empty? && all_child_events.first.context.is_a?(User) && user_json_is_admin?(@context, current_principal)
      end
  ...
  def common_ag_context_codes(appointment_group, current_principal, event, for_scheduler)
    codes_for_user = appointment_group.context_codes_for_user(current_principal)
