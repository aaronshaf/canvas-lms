...
module Lti
  ...
  class UsersApiController < ApplicationController
    ...
    # @returns User
    def show
      render json: user_json(user, user.principal, nil, [], tool_proxy.context, tool_includes: USER_INCLUDES)
    end
    ...
    def group_index
      ...
      render json: users.map { |user| user_json(user, user.principal, nil, [], group.context, tool_includes: USER_INCLUDES) }
    end
