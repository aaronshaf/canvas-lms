  ...
  def roster
    ...
    when Course
      ...
      js_permissions = {
        ...
        can_add_differentiation_tags: @context.grants_right?(current_principal, :manage_tags_add),
        can_edit_differentiation_tags: @context.grants_right?(current_principal, :manage_tags_manage),
        can_delete_differentiation_tags: @context.grants_right?(current_principal, :manage_tags_delete),
    ...
    when Group
      ...
      if (course = @context.context.is_a?(Course) && @context.context)
        ...
        @secondary_users = { t("roster.teachers_and_tas", "Teachers & TAs") => instructors.select { |instructor| UserSearch.scope_for(course, current_principal).include?(instructor) } }
      end
    end
